...
function initAdmin() {
  ...
  if (!projectId) {
    throw new Error(
      "FIREBASE_PROJECT_ID ou NUXT_PUBLIC_FIREBASE_PROJECT_ID deve estar definido.",
    );
  }
  ...
  console.log(
    `Credencial: ${clientEmail && privateKey ? "service account do .env" : "Application Default Credentials"}`,
  );
...
async function main() {
  ...
  const config = JSON.parse(
    await readFile(
      new URL("../server/data/config.json", import.meta.url),
      "utf-8",
    ),
  );
  ...
  for (const questao of defs.questoes || []) {
    const questionRef = marathonRef
      .collection("questions")
      .doc(String(questao.id));
  ...
  console.log(
    `Seed concluido: marathons/default com ${defs.questoes?.length || 0} questoes.`,
  );
}
...
main().catch((error) => {
  if (
    ...
    console.error(
      "Configure FIREBASE_CLIENT_EMAIL e FIREBASE_PRIVATE_KEY no .env, ou rode `gcloud auth application-default login`.",
    );
  } else {
    console.error(error);
  }
