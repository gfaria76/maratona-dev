...
describe("access-control", () => {
  ...
  it("scores security events by severity", () => {
    expect(
      getSecurityScore({ focusLosses: 0, fullscreenExits: 0, largePastes: 0 }),
    ).toBe("normal");
    expect(
      getSecurityScore({ focusLosses: 2, fullscreenExits: 1, largePastes: 0 }),
    ).toBe("attention");
    expect(
      getSecurityScore({ focusLosses: 1, fullscreenExits: 3, largePastes: 1 }),
    ).toBe("critical");
  });
  ...
  it("requires an active session for protected exam actions", () => {
    ...
    expect(hasActiveSessionForUserIp(sessions, "u1", "192.168.0.10")).toBe(
      true,
    );
    expect(hasActiveSessionForUserIp(sessions, "u1", "192.168.0.11")).toBe(
      false,
    );
    expect(
      hasActiveSessionForUserIp(
