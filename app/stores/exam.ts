...
import { executarPythonLocal } from "~/utils/local-python-runner";
...
export const useExamStore = defineStore("exam", () => {
  ...
  const validacoes = ref<Record<number, ValidacaoLocal>>({});
  ...
  const questaoAtual = computed(
    () => questoes.value[questaoAtualIdx.value] ?? null,
  );
  ...
  const tempoCritico = computed(
    () => tempoRestante.value > 0 && tempoRestante.value <= 600,
  );

  async function verificarAcesso() {
    const result = await $fetch<AccessVerifyResponse>("/api/access/verify", {
      method: "POST",
    });
  ...
  async function carregarQuestoes() {
    ...
    for (const q of questoes.value) {
      ...
      if (!validacoes.value[q.id]) {
        validacoes.value[q.id] = createValidacaoLocal(q);
      }
    }
  }
  ...
  function atualizarCodigo(questaoId: number, codigo: string) {
    ...
    const atuais = arquivos.value[questaoId] ?? [
      { name: "main.py", content: codigo },
    ];
    arquivos.value = {
      ...arquivos.value,
  ...
  function atualizarArquivos(
    questaoId: number,
    novosArquivos: CodigoArquivo[],
  ) {
    const normalized = normalizeArquivos(novosArquivos);
    arquivos.value = { ...arquivos.value, [questaoId]: normalized };
    const main =
      normalized.find((arquivo) => arquivo.name === "main.py") ?? normalized[0];
    codigos.value = { ...codigos.value, [questaoId]: main?.content ?? "" };
  }

  function atualizarValidacaoLocal(
    questaoId: number,
    fields: Partial<ValidacaoLocal>,
  ) {
    const current = getValidacaoLocal(questaoId);
    validacoes.value = {
      ...validacoes.value,
      [questaoId]: {
        ...current,
  ...
  async function submeter(
    questaoId: number,
  ): Promise<ResultadoCorrecao | null> {
    ...
    try {
      const resultado = await $fetch<ResultadoCorrecao>("/api/submeter", {
        ...
        body: {
          questaoId,
          codigo,
          arquivos: arquivos.value[questaoId] ?? [
            { name: "main.py", content: codigo },
          ],
        },
      });
  ...
  async function executar(
    ...
    codigo: string,
    inputs?: string,
    codigoArquivos: CodigoArquivo[] = [],
    codigoTeste?: string,
  ) {
    executando.value = true;
    atualizarValidacaoLocal(questaoId, {
    ...
    try {
      const resultado = await executarPythonLocal({
        codigo,
        codigoTeste,
        stdin: inputs,
        arquivos: codigoArquivos,
      });
      atualizarValidacaoLocal(questaoId, {
      ...
      atualizarValidacaoLocal(questaoId, {
        output: "",
        erro: getErrorMessage(e, "Erro ao executar."),
      });
    } finally {
  ...
  async function reportarEventoSeguranca(
    eventType: string,
    ...
    try {
      const result = await $fetch<{ ok: boolean; severity: SecurityScore }>(
        "/api/access/events",
        {
          method: "POST",
          body: {
  ...
  function normalizeArquivos(novosArquivos: CodigoArquivo[]): CodigoArquivo[] {
    ...
    const mainIndex = safeFiles.findIndex(
      (arquivo) => arquivo.name === "main.py",
    );
    if (mainIndex === -1) {
      return [
        { name: "main.py", content: safeFiles[0]?.content ?? "" },
        ...safeFiles,
      ];
    }
  ...
  function createValidacaoLocal(questao: QuestaoPublica): ValidacaoLocal {
    return {
      codigoTeste: questao.codigo_teste,
      stdin: questao.stdin,
      output: "",
      erro: undefined,
    };
  ...
  function getValidacaoLocal(questaoId: number): ValidacaoLocal {
    const existing = validacoes.value[questaoId];
    if (existing) return existing;

    const questao = questoes.value.find((q) => q.id === questaoId);
    return questao
      ? createValidacaoLocal(questao)
      : { codigoTeste: "", stdin: "", output: "", erro: undefined };
  }
