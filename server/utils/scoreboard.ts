  ...
  for (const submission of submissions) {
    ...
    if (!current || isBetterSubmission(percent, submittedAt, current)) {
      row.questions[submission.questao_id] = {
        ...
        status: String(
          submission.status || (percent === 100 ? "correta" : "incorreta"),
        ),
        submittedAt,
      };
    }
...
function finalizeRow(
  ...
  const scores = questions.map(
    (question) => row.questions[question.id]?.percent || 0,
  );
  const correctDates = questions
    .map((question) => row.questions[question.id])
    .filter((score): score is QuestionScore =>
      Boolean(score && score.percent === 100 && score.submittedAt),
    )
  ...
  row.averagePercent = scores.length
    ? Math.round(
        scores.reduce((sum, percent) => sum + percent, 0) / scores.length,
      )
    : 0;
  row.lastSubmissionAt = latestDate(row._submissionDates);
  row.tieBreakAt =
    row.totalCorrect > 0 ? latestDate(correctDates) : row.lastSubmissionAt;
...
function compareRows(a: ScoreboardRow, b: ScoreboardRow) {
  ...
  const aTie = a.tieBreakAt
    ? Date.parse(a.tieBreakAt)
    : Number.POSITIVE_INFINITY;
  const bTie = b.tieBreakAt
    ? Date.parse(b.tieBreakAt)
    : Number.POSITIVE_INFINITY;
