...
export async function requireExamUser(
  event: H3Event,
): Promise<CurrentExamUser> {
  const token =
    getBearerToken(event) || getCookie(event, "firebase_id_token") || "";
  if (!token) {
  ...
  if (!isUfmsEmail(email)) {
    throw createError({
      statusCode: 403,
      message: "Apenas emails @ufms.br são permitidos.",
    });
  }
