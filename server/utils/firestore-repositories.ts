...
export async function getQuestionWithTests(
  marathonId: string,
  questaoId: number,
  ...
  const snapshot = await questionsCollection(marathonId)
    ...
    .get();
  const questionDoc =
    snapshot.docs[0] || (await fallbackQuestionDoc(marathonId, questaoId));
...
export async function listAllowedIpRanges(
  marathonId: string,
): Promise<FirestoreAllowedIpRange[]> {
  const snapshot = await marathonCollection(
    marathonId,
    "allowedIpRanges",
  ).get();
  return snapshot.docs.map(
    (doc) => ({ id: doc.id, ...doc.data() }) as FirestoreAllowedIpRange,
  );
}
...
export async function listActiveSessions(
  ...
  const snapshot = await marathonCollection(marathonId, "sessions")
    ...
    .get();
  return snapshot.docs.map(
    (doc) => ({ id: doc.id, ...doc.data() }) as FirestoreSession,
  );
}
...
export async function listActiveBlocks(
  ...
  const snapshot = await marathonCollection(marathonId, "ipBlocks")
    ...
    .get();
  return snapshot.docs.map(
    (doc) => ({ id: doc.id, ...doc.data() }) as FirestoreIpBlock,
  );
}
...
export async function updateSessionsStatus(
  ...
  for (const sessionId of sessionIds) {
    batch.set(
      marathonCollection(marathonId, "sessions").doc(String(sessionId)),
      {
        status,
        last_seen_at: date,
        ...fields,
...
export async function countSecurityEvents(
  ...
  return snapshot.docs.filter((doc) =>
    eventTypes.includes(String(doc.data().event_type)),
  ).length;
}

export async function createSecurityEvent(
  marathonId: string,
  event: FirestoreSecurityEvent,
) {
...
export async function clearSecurityEvents(
  ...
  const snapshot = await marathonCollection(marathonId, "securityEvents").get();
  const cutoff =
    filter.olderThanHours && filter.olderThanHours > 0
  ...
  const docs = snapshot.docs.filter((doc) => {
    ...
    if (
      ...
      return false;
    if (cutoff && Date.parse(String(data.created_at || "")) >= cutoff)
      return false;
    return true;
  });
...
function normalizeMarathon(
  ...
  return {
    ...
    duracao_minutos: Number(
      data.duracao_minutos || data.durationMinutes || 180,
    ),
    mostrar_detalhes_testes: Boolean(
      data.mostrar_detalhes_testes ?? data.showTestDetails ?? false,
    ),
    mensagem_boas_vindas: String(
      data.mensagem_boas_vindas || data.welcomeMessage || "",
    ),
...
function normalizeQuestion(
  ...
  return {
    ...
    funcao_assinatura: String(
      data.funcao_assinatura ||
        data.functionSignature ||
        "def resolver(entrada):\n    pass",
    ),
    codigo_teste: String(data.codigo_teste || data.validationCode || ""),
    stdin: String(data.stdin || data.validationStdin || ""),
    impedimentos: Array.isArray(data.impedimentos)
      ? data.impedimentos
      : Array.isArray(data.impediments)
        ? data.impediments
        : [],
...
async function fallbackQuestionDoc(marathonId: string, questaoId: number) {
  const doc = await questionsCollection(marathonId)
    .doc(String(questaoId))
    .get();
