...
export function evaluateAccessAttempt(attempt: AccessAttempt): AccessDecision {
  const activeSessions = attempt.activeSessions.filter(
    (session) => session.status === "active",
  );
  const currentIpSessions = activeSessions.filter(
  ...
  if (!isIpAllowed(attempt.ipAddress, attempt.allowedRanges)) {
    return deny(
      "ip_not_allowed",
      currentIpSessions.map((session) => session.id),
    );
  }
  ...
  const sameUserSessions = activeSessions.filter(
    (session) => session.userId === attempt.userId,
  );
  ...
  if (sameUserOtherIpSessions.length > 0) {
    return {
      ...
      ipsToBlock: unique([
        attempt.ipAddress,
        ...sameUserOtherIpSessions.map((session) => session.ipAddress),
      ]),
      conflictingSessionIds: sameUserOtherIpSessions.map(
        (session) => session.id,
      ),
    };
  }
  ...
  if (sameIpOtherUserSessions.length > 0) {
    return {
      ...
      conflictingSessionIds: sameIpOtherUserSessions.map(
        (session) => session.id,
      ),
    };
  }
...
export function getSecurityScore(counts: SecurityEventCounts): SecurityScore {
  const score =
    counts.focusLosses + counts.fullscreenExits * 2 + counts.largePastes * 3;
