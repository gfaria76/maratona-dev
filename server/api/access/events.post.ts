...
export default defineEventHandler(async (event) => {
  ...
  if (
    ...
    throw createError({
      statusCode: 400,
      message: "Evento de segurança inválido.",
    });
  }
  ...
  const [focusLosses, fullscreenExits, largePastes] = await Promise.all([
    countSecurityEvents(marathon.id, body.sessionId, [
      "visibility_hidden",
      "window_blur",
    ]),
  ...
  const nextCounts = {
    focusLosses:
      focusLosses +
      (["visibility_hidden", "window_blur"].includes(body.eventType) ? 1 : 0),
    fullscreenExits:
      fullscreenExits + (body.eventType === "fullscreen_exit" ? 1 : 0),
