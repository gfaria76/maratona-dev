 ...
 */
import type { CodigoArquivo, ResultadoCorrecao, ResultadoTeste } from '#shared/types/exam'
import { validarImpedimentos, executarTestes } from '../utils/python-runner'
import { getRequestIp, requireExamUser, type CurrentExamUser } from '../utils/server-auth'
import {
  createSecurityEvent,
  createSubmission,
  findActiveSessionId,
...
export default defineEventHandler(async (event): Promise<ResultadoCorrecao> => {
  ...
  const body = await readBody<{ questaoId: number; codigo: string; arquivos?: CodigoArquivo[] }>(event)

  if (!body?.questaoId || !body?.codigo) {
    throw createError({ statusCode: 400, message: 'Campos "questaoId" e "codigo" são obrigatórios.' })
  }

  const questao = await getQuestionWithTests(marathon.id, body.questaoId)

  if (!questao) {
    throw createError({ statusCode: 404, message: `Questão ${body.questaoId} não encontrada.` })
  }
  ...
  const codigoCompleto = [body.codigo, ...arquivos.map((arquivo) => arquivo.content)].join('\n')
  const impedimentosViolados = validarImpedimentos(codigoCompleto, questao.impedimentos)
  ...
  const resultadosTestes = await executarTestes(body.codigo, questao.funcao_nome, questao.testes, arquivos)
  ...
  const executionError = detalhes.find((detalhe) => detalhe.erro)?.obtido
    || detalhes.find((detalhe) => detalhe.erro)?.saida
    || null
  const status = hasExecutionError ? 'erro' : acertos === total ? 'correta' : 'incorreta'
